...
class FacebookRequest {
    ...
    func fetchFbPostsFrom(page: FacebookPage, nextEdge: String?, parameters: [NSObject: AnyObject]?, handler: ([FacebookPost], String) -> Void)  {
        ...
        var feedURL: NSURL?
        if nextEdge == nil {
            feedURL = NSURL(string:  pageGraphUrl + FacebookRequest.kGraphFeed)
            print(feedURL)

        } else {
            feedURL = NSURL(string:  FacebookRequest.kGraphURL + nextEdge! + "/" + FacebookRequest.kGraphFeed)
            print(feedURL)
        ...
        fetchWithFunction(slRequest) { results in
            do {
                ...
                if let nextPage = feed.valueForKeyPath("paging.next") as? String {
                    next = nextPage
                }
